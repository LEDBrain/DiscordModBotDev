...
module.exports = {
    do: function(params) {
        ...
        if (!member) {
            params.message.channel.send(`Bitte gebe ein User an! Format: \`${params.prefix}mute <@user> <Grund>\``);
            return;
        }

        if (!reason) {
            params.message.channel.send(`Bitte gebe einen Grund an! Format: \`${params.prefix}mute <@user> <Grund>\``);
            return;
        }
