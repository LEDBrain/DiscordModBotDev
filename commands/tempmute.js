...
module.exports = {
    do: async function(params) {
        ...
        if (!member) return params.message.channel.send(`Bitte gebe ein User an! Format: \`${config.prefix}tempmute <@user> <Grund>\``);

        if (!mutetime) return params.message.reply("Du musst eine Dauer angeben!");

        if (!reason) return params.message.channel.send(`Bitte gebe einen Grund an! Format: \`${config.prefix}tempmute <@user> <Grund>\``);
