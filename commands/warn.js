...
module.exports = {
    do: function(params) {
        ...
        db.query("SELECT `warns` FROM `warnungen` WHERE `id` = ?", [member.id], (err, result) => {
            ...
            let sql = result[0] ? mysql.format("INSERT INTO `warnungen` (`id`, `username`, `warns`) VALUE (?, ?, ?)", [member.id, member.user.username, warns]): mysql.format("UPDATE `warnungen` SET `warns` = ? WHERE `id` = ?", [warns, member.id]);
            db.query(sql, error => {
